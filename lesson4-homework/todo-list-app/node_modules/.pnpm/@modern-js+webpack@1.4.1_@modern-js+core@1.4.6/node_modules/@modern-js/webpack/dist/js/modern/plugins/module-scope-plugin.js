...
export class ModuleScopePlugin {
  constructor({
    ...
    this.relativeAllowedDirs = this.allowedDirs.map(allowedDir => `${path.relative(path.dirname(allowedDir), allowedDir)}/`);
  }
  ...
  apply(resolver) {
    ...
    resolver.hooks.file.tapAsync('ModuleScopePlugin', (request, contextResolver, callback) => {
      ...
      if (allowedDirs.every(allowedDir => {
        const relative = path.relative(allowedDir, request.context.issuer); // If it's not in one of our app src or a subdirectory, not our request!
      ...
      const requestFullPath = path.resolve(path.dirname(request.context.issuer), request.__innerRequest_request); // allowd pattern
      ...
      if (allowedDirs.every(allowedDir => {
        ...
        let message = `You attempted to import ${chalk.bold(request.__innerRequest_request)} which is not allowed. `;

        if (allowedDirs.length) {
          message += `Allowed dirs: ${chalk.bold(relativeAllowedDirs.join(','))}. `;
        }

        if (allowedPatterns.length) {
          message += `Allowed patterns: ${chalk.bold(allowedPatterns.map(p => p.toString()).join(','))}. `;
        }
