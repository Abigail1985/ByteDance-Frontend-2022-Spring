...
const verifyTsConfigPaths = (root, userConfig) => {
  ...
  const paths = Object.keys(((_readTsConfig$compile = (0, _utils.readTsConfig)(root).compilerOptions) === null || _readTsConfig$compile === void 0 ? void 0 : _readTsConfig$compile.paths) || {}).map(key => key.replace(/\/\*$/, ''));
  Object.keys(userAliases).forEach(name => {
    if (paths.includes(name)) {
      throw new Error(_utils.chalk.red(`It looks like you have configured the alias ${_utils.chalk.bold(name)} in both the modern.config file and tsconfig.json.\n Please remove the configuration in modern.config file and just keep the configuration in tsconfig.json.`));
    }
  });
};

const getWebpackAliases = (appContext, userConfig) => {
  if ((0, _utils.isTypescript)(appContext.appDirectory)) {
