...
const getMessage = (schema, answers, extra) => answer => {
...
const getDefaultValue = (nodeInfo, answers) => {
  var _nodeInfo$state, _nodeInfo$state3;

  if ((0, _easyFormCore.isEffectedValue)((_nodeInfo$state = nodeInfo.state) === null || _nodeInfo$state === void 0 ? void 0 : _nodeInfo$state.value)) {
...
const getChoices = options => {
  ...
  return answer => {
    ...
    let result = [];
    choices.forEach(x => {
      const nodeInfo = (0, _easyFormCore.getNodeInfo)(x, answers, extra);
...
const getValidate = (schema, answers, extra) => async answer => {
...
const getFilter = (nodeInfo, type, answers) => {
...
const getTransformer = (nodeInfo, answers) => answer => {
...
const getWhen = (nodeInfo, answers) => answer => {
...
const toPromiseQuestionHandler = options => {
  const {
    schema,
    nodeInfo,
    type,
...
exports.toPromiseQuestionHandler = toPromiseQuestionHandler;

const toPromiseQuestionHandlerLoop = questionHandlers => answers => // The root node itself is not a problem, so the sub-elements question are executed in order
new Promise(resolve => {
  try {
...
exports.toPromiseQuestionHandlerLoop = toPromiseQuestionHandlerLoop;