...
import { BaseReader, getItems, toBoolean, getSchemaDefaultState, getNodeInfo } from '@modern-js/easy-form-core';
...
export class BaseCliReader {
  ...
  constructor(_options) {
    ...
    this.analyNoneNode = schema => this.cliHandler[CliNodeHandlers.ANALY_NONE_ITEM]({
      nodeInfo: getNodeInfo(schema, this.answers, this.extra)
    });

    this.analyValueNode = schema => this.cliHandler[CliNodeHandlers.ANALY_INPUT]({
      schema,
      nodeInfo: getNodeInfo(schema, this.answers, this.extra)
    });
    ...
    this.getChildNodes = (options, answers) => {
      ...
      const childNodes = [];
      getItems(parent, answers, this.extra).forEach(each => {
        if (each.items) {
    ...
    this.schemaBaseReader = new BaseReader(_options.schema, this.answers, _options.extra); // this.onChange = options.onChange;
