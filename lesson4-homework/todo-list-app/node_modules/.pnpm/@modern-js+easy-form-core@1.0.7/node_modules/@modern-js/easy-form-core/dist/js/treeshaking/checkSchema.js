...
var checkFieldType = function checkFieldType(schema) {
  ...
  if (schema.state && !['string', 'object'].includes(_typeof(schema.state))) {
...
var checkRelation = function checkRelation(schema) {
...
var checkKeyExist = function checkKeyExist(schema) {
  if (!schema.key) {
...
var checkRepeatItems = function checkRepeatItems(schema, extra) {
  if (schema.items && schema.items.length > 0) {
    ...
    if (repeatKeys.length > 0) {
      throw Error("schema(".concat(schema.key, ") items has repeat keys: ").concat(repeatKeys.join(',')));
    }
  }
};

export var checkSchema = function checkSchema(schema, extra) {
  // Key with value
  var fieldKeys = []; // Duplicate fieldKeys

  var repeatFieldKeys = []; // Check the key of the global field with value, it cannot be repeated, the value of the form will be overwritten after repeated

  var checkRepeatFieldKeys = function checkRepeatFieldKeys(_schema) {
  ...
  forEach(schema, function (_schema) {
