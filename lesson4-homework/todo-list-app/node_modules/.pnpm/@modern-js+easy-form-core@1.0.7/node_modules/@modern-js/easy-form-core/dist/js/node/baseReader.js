...
class BaseReader {
  ...
  constructor(schema, data, extra) {
    ...
    if (!schema) {
      throw Error(_constant.MESSAGE.SCHEMA_SHOULD_NOT_BE_EMPTY);
    }
  ...
  registReadRoot(callback) {
    this.rulesHandler[_constant.NODE_HANDLERS.READ_ROOT] = callback;
    return {
  ...
  registReadForm(callback) {
    this.rulesHandler[_constant.NODE_HANDLERS.READ_FORM] = callback;
    return {
  ...
  registReadChild(callback) {
    this.rulesHandler[_constant.NODE_HANDLERS.READ_CHILD] = callback;
    return {
  ...
  registReadAsValue(callback) {
    this.rulesHandler[_constant.NODE_HANDLERS.READ_AS_VALUE] = callback;
    return {
  ...
  registReadCoexitRelation(callback) {
    this.rulesHandler[_constant.NODE_HANDLERS.READ_COEXIT_RELATION] = callback;
    return {
  ...
  registerReadNoneItem(callback) {
    this.rulesHandler[_constant.NODE_HANDLERS.READ_NONE_ITEM] = callback;
    return {
  ...
  registReadMutualexclusionRelation(callback) {
    this.rulesHandler[_constant.NODE_HANDLERS.READ_MUTUALEXCLUSION_RELATION] = callback;
    this.setReady();
  }
  ...
  getInitValues(schema) {
    ...
    const readDefaultValue = (_schema, brothers, isRoot) => {
      ...
      if (_schema.items) {
        const items = (0, _utils.getItems)(_schema, this.data, this.extra);
  ...
  getSchemaDefaultValue(schema, brothers, isRoot) {
    ...
    if (typeof state === 'object') {
      ...
      const handleEffects = val => {
        if (!(0, _utils.isEffectedValue)(val)) {
  ...
  read() {
    ...
    (0, _checkSchema.checkSchema)(this.schema, this.extra);
    return this.runHandler(_constant.NODE_HANDLERS.READ_ROOT, {
      schema: this.schema,
  ...
  readObj(schema) {
    ...
    for (const item of (0, _utils.getItems)(schema, this.data, this.extra)) {
      if (item.items) {
        children.push(this.doRead(item));
      } else if (this.isNoneItem(item)) {
        children.push(this.runHandler(_constant.NODE_HANDLERS.READ_NONE_ITEM, item));
      } else {
        children.push(this.runHandler(_constant.NODE_HANDLERS.READ_AS_VALUE, item));
      }
    }
  ...
  readRelation(schema) {
    if (schema.coexit) {
      return this.runHandler(_constant.NODE_HANDLERS.READ_COEXIT_RELATION, {
        parent: schema,
    ...
    if (schema.mutualExclusion) {
      return this.runHandler(_constant.NODE_HANDLERS.READ_MUTUALEXCLUSION_RELATION, {
        parent: schema,
        doRead: this.doRead.bind(this)
      });
    }

    throw new Error(_constant.MESSAGE.SHOULD_HAS_RELATION(''));
  }
  ...
  doRead(schema, root) {
    ...
    if (!schema.items) {
      if (this.isNoneItem(schema)) {
        result = this.runHandler(_constant.NODE_HANDLERS.READ_NONE_ITEM, schema);
      } else {
        result = this.runHandler(_constant.NODE_HANDLERS.READ_AS_VALUE, schema);
      }
    } else if (schema.isObject) {
      result = this.readObj(schema);

      if (root) {
        return this.runHandler(_constant.NODE_HANDLERS.READ_FORM, {
          schema,
          result
        });
      } else {
        return this.runHandler(_constant.NODE_HANDLERS.READ_CHILD, {
          schema,
          result
        });
      }
