...
export const getRouteName = (resourcePath, apiDir) => {
  ...
  if (resourcePath.startsWith(lambdaDir)) {
    ...
    const draftName = relativeName.split(path.sep).map(item => {
      if (item.length > 2) {
    ...
    const name = draftName.endsWith(INDEX_SUFFIX) ? draftName.substring(0, draftName.length - INDEX_SUFFIX.length - 1) : draftName;
    return Ok(name);
  } else {
