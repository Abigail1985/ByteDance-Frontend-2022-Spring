...
export const getPresetChain = option => {
  const {
    appDirectory,
    presets: {
      envOptions,
  ...
  const chain = createBabelChain(); // set envOptions = false
  ...
  if (!disableEnvPreset) {
    const browsersTargets = syntax === 'es5' ? getBrowserslist(appDirectory) : es6BrowserList;
