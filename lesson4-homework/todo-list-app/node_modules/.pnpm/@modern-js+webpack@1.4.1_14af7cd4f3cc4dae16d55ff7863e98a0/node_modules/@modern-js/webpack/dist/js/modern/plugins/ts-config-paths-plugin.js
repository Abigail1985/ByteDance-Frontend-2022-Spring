...
import { logger, readTsConfig, isRelativePath, createDebugger } from '@modern-js/utils';
...
export class TsConfigPathsPlugin {
  constructor(cwd) {
    ...
    this.absoluteBaseUrl = path.resolve(cwd, this.compilerOptions.baseUrl || './');
    this.matchPath = createMatchPath(this.absoluteBaseUrl, ((_this$compilerOptions = this.compilerOptions) === null || _this$compilerOptions === void 0 ? void 0 : _this$compilerOptions.paths) || {}, ['browser', 'module', 'main'], false);
    this.resolved = new Map();
  }

  apply(resolver) {
    if (!resolver) {
      logger.warn('ts-config-paths-plugin: Found no resolver, not apply ts-config-paths-plugin');
    }
    ...
    resolver.getHook('described-resolve').tapAsync('TsConfigPathsPlugin', (request, resolveContext, callback) => {
      ...
      if (!this.resolved.has(requestName)) {
        const matched = this.matchPath(requestName, undefined, undefined, JS_RESOLVE_EXTENSIONS);
