...
export const getPackageConfig = (appDirectory, packageJsonConfig) => {
...
export const loadConfig = async (appDirectory, filePath, packageJsonConfig) => {
  const configFile = filePath ? filePath : findExists(CONFIG_FILE_EXTENSIONS.map(extension => path.resolve(appDirectory, `${CONFIG_FILE_NAME}${extension}`)));
  const pkgConfig = getPackageConfig(appDirectory, packageJsonConfig);
  let config;
  const dependencies = pkgConfig ? [path.resolve(appDirectory, './package.json')] : [];
