...
async function isValidCache(cacheDir) {
  ...
  if (await fsExists(`${cacheDir}/.codesmith.completed`)) {
    const preCacheTimeStr = await fs.readFile(`${cacheDir}/.codesmith.completed`, {
      encoding: 'utf-8'
    });
...
async function downloadAndDecompressTargz(tarballPkg, targetDir) {
  ...
  await new Promise((resolve, reject) => {
    fs.createReadStream(tempTgzFilePath).pipe(tar.x({
      strip: 1,
      C: `${targetDir}`
    })).on('finish', () => {
      resolve();
...
export async function downloadPackage(pkgName, pkgVersion = 'latest', options = {}) {
  const {
    registryUrl,
    install
  ...
  await fs.mkdirp(targetDir); // get package tarball
  ...
  await fs.writeFile(`${targetDir}/.codesmith.completed`, new Date().toISOString(), {
    encoding: 'utf-8'
  });
  return targetDir;
}